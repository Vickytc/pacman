...
class PacmanGraphics:
    ...
    def drawAgentObjects(self, state):
        ...
        for index, agent in enumerate(state.agentStates):
            if self.MAPF:
                index, agent = agent[0], agent[1]
                image = self.drawPacman(agent, index)
                self.agentImages.append((agent, image, index))
            elif agent.isPacman:
                image = self.drawPacman(agent, index)
    ...
    def update(self, newState):
        ...
        if self.MAPF:
            process_array = []
            for agent, agentState in newState.agentStates:
                for k, (s, i, index) in enumerate(self.agentImages):
                    if index == agent:
                        process_array.append((agentState, s, i))
                        self.agentImages[k] = (agentState, i, index)
                        break
            ...
            for i in range(1, int(frames) + 1):
                for pacman, prevPacman, image in process_array:
    ...
    def drawPacman(self, pacman, index):
        ...
        if self.MAPF:
            outlineColor = GHOST_COLORS[ord(index) - ord('A')]
            fillColor = GHOST_COLORS[ord(index) - ord('A')]
                ...
                if cell:  # There's food here
                    ...
                    if self.MAPF:
                        dot = circle(screen,
                                     ...
                                     outlineColor=GHOST_COLORS[ord(cell)-ord('A')], fillColor=GHOST_COLORS[ord(cell)-ord('A')],
                                     width=1)
                    else:
                        dot = circle(screen,
                                     FOOD_SIZE * self.gridSize,
